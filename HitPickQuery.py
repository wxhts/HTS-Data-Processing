...
with open(output_path, 'wb') as output_file:
    ...
    if structures == 'Y':
        findcompound = c.execute(
            "SELECT EchoSource.Barcode, EchoSource.Well, EchoSource.Client_ID, ChinaSource.SMILES FROM EchoSource INNER JOIN ChinaSource ON EchoSource.Client_ID=ChinaSource.Client_ID WHERE EchoSource.Client_ID IN " + cmpd)
